...
Nous commençons par définir les notions de *filtre* et de *système linéaire invariant*.
...
Un filtre est un *système*, c'est-à-dire un processus qui transforme un signal d'entrée $x$ en un signal de sortie $y$.
...
Nous ne considérons dans ce cours que des filtres *linéaires* et *invariants*.
...
supprime les hautes fréquences afin d'éviter les problèmes de repliement spectral (ou *aliasing*) lors de l'échantillonnage.
...
Cela signifie que les signaux qui composent l'entrée du système sont traitées par celui-ci de façon proportionnelle et additive.
Par exemple, si l'entrée est amplifiée d'un facteur 2, alors la sortie le sera aussi.
De plus, si l'entrée est la somme de deux signaux, alors la sortie sera la somme des sorties individuelles de ces deux signaux.
...
On parle de systèmes linéaires invariant (*linear-time invariant system* ou *LTI system*).
...
* la sortie d'un système linéaire invariant s'exprime par la convolution de l'entrée
  avec la [réponse impulsionnelle](#P:filtrage:ri) du système
  (ce qui n'est pas le cas pour d'autres systèmes n'ayant pas ces deux propriétés).

✏️ @P:filtrage:exercice-1
...
On peut montrer que sa [réponse impulsionnelle](#P:filtrage:ri) est absolument sommable
...
Un filtre est caractérisé par sa réponse impulsionnelle (*impulse response*) $h$.
...
La réponse fréquentielle (*frequency response*) est la transformée de Fourier de la réponse impulsionnelle.
...
:::{figure} figs/intro-gain.svg
:width: 450px
Gain du filtre représenté @F:filtrage:intro:repimp.
:::
...
:::{figure} figs/intro-phase.svg
:width: 450px
Phase du filtre représenté @F:filtrage:intro:repimp.
:::
...
:::{figure} figs/intro-retard-groupe.svg
---
width: 450px
---
Retard de groupe du filtre représenté @F:filtrage:intro:repimp.
:::
...
sont mises en entrée du filtre représenté @F:filtrage:intro:repimp.
